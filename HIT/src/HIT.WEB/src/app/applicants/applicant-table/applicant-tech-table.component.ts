...
import { TechApplicantDataSource } from 'src/app/core/services/applicants/tech-datasource';
...
export class TechApplicantTableComponent implements OnInit, AfterViewInit {
  datasource!: TechApplicantDataSource;
  ...
  ngOnInit(): void {
    this.datasource = new TechApplicantDataSource(this.applicationService);
...
next(data: JobApplication) {
  this.jobAppService.UpdatePhase(data.id!, 2).subscribe((res) => {
    let jobs: JobApplication[] = this.applicationService.TECHapplications$$.value;
