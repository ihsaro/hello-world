...
next(data: JobApplication) {
  this.jobAppService.UpdatePhase(data.id!, 3).subscribe((res) => {
    ...
    this.applicationService.Contractapplications$$.next([{...data, applicationPhase: ApplicationPhase.READY_FOR_CONTRACT},  ...this.applicationService.HRapplications$$.value])
  })
...
back(data: JobApplication) {
  this.jobAppService.UpdatePhase(data.id!, 0).subscribe((res) => {
    ...
    jobs.splice(index, 1)
    this.applicationService.HRapplications$$.next(jobs)
    this.applicationService.TECHapplications$$.next([{...data, applicationPhase: ApplicationPhase.READY_FOR_TECHNICAL_INTERVIEW},  ...this.applicationService.Entryapplications$$.value])
  })
